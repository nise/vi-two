	...
	var TemporalTagging = $.inherit(
	{
		...
		displayTagcloud : function(){
			...
			$.each(this.tag_obj, function(i, val){ 
				ul.append(ln(val.name, val.occ.length, val));
			});
		...
		showTimelineTag : function(e){
			...
			if(e.tags.occ.length === 1){
  	    	...
  	    	if (isNaN(progress)) { progress = 0; }
	 				$(_this.timelineSelector).append(f(progress, e.tags.name, this));
 				});
			}
		},
		...
		saveTags : function(){
			...
			$.each($('ul#mytags > li'), function(i, val){
				...
				//$('#debug').append("{name:'"+$(val).find('input[type=hidden]').val()+"', occ:["+_this.player.currentTime()+"]}");
